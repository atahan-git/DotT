...
public class Minion : MonoBehaviour{
	...
	NavMeshAgent agent; //The NavMeshAgent
	...
	void FixedUpdate(){
		if(attackFocus != null){
			...
			if(attackCooldownCurrent == 0){
				Attack(attackFocus.gameObject, damage);
		...
		if(!paused){//If pathfinding hasn't been paused,
			...
			foreach(Transform enemyTower in enemyTowers){
				...
				if(distance < closestDistanceEnemyTower && distance < towerTrackRange){
					if(enemyTower.GetComponent<TowerController>().alive && enemyTower.gameObject.activeSelf){
	...
	*/
	virtual public void Attack(){
		print("pew");
		attackFocus.gameObject.GetComponent<Health>().Damage(damage);
	}
